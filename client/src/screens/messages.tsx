...
import { FaCircleArrowRight, FaSlideshare } from "react-icons/fa6";
...
function View({
      ...
      axios
        .get(
          process.env.REACT_APP_BACKEND_URL +
            "/interviews/all-interviews-by-interviewer/" +
            _id
        )
        .then((res: any) => {
          if (res?.data?.length) {
            ...
            let _interviewees = allInterviewsByInterviewer
              .map((a: any) => a?.interviewee)
              .filter((v: any) => v);
            _interviewees = Array.from(
              new Set(
                _interviewees.map((_interviewee: any) => _interviewee._id)
              )
            ).filter((i: any) => i !== user?.id);
            ...
            if (_interviewees?.length) {
              ...
              try {
                ...
                if (user) {
                  ...
                  client.connectUser({ id: user?.id }, user?.chat.token);
  ...
  const getInterviewee = (id: any) => {
    if (interviewsByInterviewer?.length) {
      const _iv = interviewsByInterviewer.find(
        (ini: any) => ini?.interviewee?._id === id
      );
      if (_iv) {
    ...
    <div className="pageContainer">
      <div className="leftSideDiv">
        ...
        <div style={{ position: "absolute", top: 0, width: "100%" }}>
          ...
          <div
            className="message message-content row m-0"
            style={{ height: 620 }}
          >
                ...
                return (
                  <div
                    ...
                    className={`message-room ${
                      interviewee === currentInterviewee?._id
                        ? "selected shadow-sm"
                        : ""
                    }`}
                    onClick={() => {
            ...
            <Card className="col-7 message-right h-100">
                          ...
                          <div key={item?.id}>
                            <div
                              className={`d-flex justify-content-${
                                item?.sent_by === user?.id ? "end" : "start"
                              }`}
                            >
                              <div
                                className={`alert alert-${
                                  item?.sent_by === user?.id ? "light" : "info"
                                } p-1 px-2 mb-0`}
                                style={{
                                  fontSize: "80%",
                                  width: "fit-content",
              ...
              <Card.Footer className="message-footer d-flex">
                ...
                <Button variant="light">
                  <FaCircleArrowRight color="gray" onClick={sendMessage} />
                </Button>
              </Card.Footer>
            </Card>
          </div>
