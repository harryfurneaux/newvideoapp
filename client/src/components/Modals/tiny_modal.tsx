...
const TinyModal = ({
  ...
  const { user, logout, setShowLoading } = useAuth();
      ...
      case "delete_account":
        return {
          ...
          onConfirm: () => {
            setShowLoading(true);
            axios
              .delete(`${process.env.REACT_APP_BACKEND_URL}/users/${user?.id}`)
              .then((res) => {
      ...
      case "delete_interview_video":
        return {
          ...
          onConfirm: () => {
            ...
            axios
              .delete(
                `${process.env.REACT_APP_BACKEND_URL}/interviews/${jobView?.id}`
              )
              .then((res) => {
                setShowLoading(false);
                handleClose();
      ...
      case "delete_question":
        return {
          ...
          onConfirm: () => {
            ...
            axios
              .delete(
                `${process.env.REACT_APP_BACKEND_URL}/questions/${jobView?._id}`
              )
              .then((res) => {
                setShowLoading(false);
                handleClose();
  ...
  return (
    <Modal
      className={`modal-primary tiny-modal ${getByType().classes}`}
      show={show}
      onHide={handleClose}
      centered
    >
      ...
      <Notify
        show={deleteNotify}
        title={notifyMessage}
        handleClose={() => setDeleteNotify(false)}
      />
    </Modal>
  );
};
