...
const PaymentSetting = ({
  ...
  useEffect(() => {
    if (user) {
      setShowLoading(true);
      axios
        .get(`${process.env.REACT_APP_BACKEND_URL}/stripe/${user?.id}`)
        .then((res) => {
  ...
  return (
    <Modal
      className={`modal-primary payment-modal ${
        show_change || show_tiny ? "z-0" : ""
      }`}
      show={show}
      onHide={handleClose}
      centered
          ...
          <div className="modal-part-body">
            <div className="d-flex align-items-start">
              ...
              <div className="modal-part-main">
                <p className="fw-bold">
                  {`${paymentData?.cardBrand} ending in ${paymentData?.last4}`}
                  &ensp;|&emsp;{paymentData?.country}
                </p>
                <p>
                  You need a primary biulling method when you have active
                  questions or a balance due. To remove this one, set a new
