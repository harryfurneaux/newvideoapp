...
const ChangeModal = ({
  ...
  const { user, setUser, setShowLoading } = useAuth();
  ...
  return (
    <Modal
      className="modal-primary change-modal"
      show={show}
      onHide={_handleClose}
      centered
    >
      ...
      <Modal.Body>
        <div
          ...
          style={{ cursor: item === "ProfilePicture" ? "pointer" : "text" }}
          onClick={(e) => {
            if (item === "ProfilePicture") {
              document.getElementById("file-upload-input")?.click();
            } else {
              const _node: HTMLInputElement | null =
                e?.currentTarget?.querySelector(`input[name="${item}"]`);
              if (_node) {
                _node.focus();
              }
            }
        ...
        <div
          ...
          onClick={(e) => {
            const _node: HTMLInputElement | null =
              e?.currentTarget?.querySelector(`input[name="current_password"]`);
            if (_node) {
              _node.focus();
            }
          }}
      ...
      <Modal.Footer>
        ...
        <Button
          className="bordered"
          onClick={handleSave}
          disabled={!(password?.length && (data?.length || file))}
        >
          Save changes
        </Button>
      </Modal.Footer>
