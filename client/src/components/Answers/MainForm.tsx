          ...
          filteredInterviews = allInterviews
            .filter((interview: any) => {
              if (interview?.createdAt) {
                ...
                return (
                  interviewDate >= startOfWeek && interviewDate <= endOfWeek
                );
              }
            })
            .filter((i: any) => i);
  ...
  return (
    <div className="leftSideMain">
      <div className="option-btn">
        <button
          className={`lamdl-anwid radiusLeft ${myAnswers ? "active" : ""}`}
          onClick={() => {
        ...
        <button
          className={`lamdl-anwid radiusRight ${myAnswers ? "" : "active"}`}
          onClick={() => setMyAnswers(false)}
        >
          <Icons iconNumber={32} />
          Nearby
        </button>
      ...
      <div className="leftSideContentAnswers">
        <div className="h-auto">
          <Row
            className="row-cols-3 row-cols-sm-4 row-cols-md-3 row-cols-lg-4 row-cols-xl-5 justify-content-start mxrow-answers"
            style={{ gap: 10, padding: "4px 14px" }}
          >
            {filteredInterviews?.length ? (
              filteredInterviews.map((interview, index) => (
                <Col
                  className="p-0 mb-0 d-inline-flex justify-content-center align-items-center"
                  style={{
                    cursor: "pointer",
                    maxWidth: "120px",
                    maxHeight: "226px",
