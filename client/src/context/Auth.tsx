...
const AuthProvider = ({ children, setMainScreen }: Props) => {
  ...
  const PaymentSuccess = (sessionId: any, userId: any) => {
    setShowLoading(true);
    axios
      .post(`${process.env.REACT_APP_BACKEND_URL}/stripe/${sessionId}`, {
        userId: userId,
      })
      .then((res) => {
  ...
  const handleRegister = (
    ...
    return new Promise((resolve, rejects) => {
      ...
      axios
        .post(
          `${process.env.REACT_APP_BACKEND_URL}${authConfig.registerEndpoint}`,
          params
        )
        .then(async (response) => {
          resolve(response);
  ...
  const handleLogin = (
    ...
    return new Promise((resolve, rejects) => {
      ...
      axios
        .post(
          `${process.env.REACT_APP_BACKEND_URL}${authConfig.loginEndpoint}`,
          params
        )
        .then(async (response) => {
          resolve(response);
